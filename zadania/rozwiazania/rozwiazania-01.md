...
[01. Liczby zespolone](../../teoria/01-liczby-zespolone.md). Wyniki sprawdzono w
numpy (Python 3.11, numpy 2.4.6).
...
**(b)** $z=-1+\sqrt3\,i$ leży w II ćwiartce, więc $\arg z=\pi-\arctan(\sqrt3/1)
=\pi-\tfrac{\pi}{3}=\tfrac{2\pi}{3}$; stąd $z=2e^{i2\pi/3}$.
...
**(b)** $\operatorname{Tr}U=\dfrac{2}{\sqrt2}=\sqrt2$, $\det U=\dfrac{1}{2}\bigl(1\cdot1-i\cdot i\bigr)
=\dfrac{1}{2}\bigl(1-i^2\bigr)=\dfrac{1}{2}(2)=1$. Jeśli $\lambda_1\lambda_2=1$ i
$\lambda_1+\lambda_2=\sqrt2=\lambda+\lambda^{-1}$, to $\lambda=e^{\pm i\pi/4}$
(numerycznie $\lambda=e^{i\pi/4},e^{-i\pi/4}$).
...
**(c)** Dla $n=5$, `np.prod(np.roots([1,0,0,0,0,-1]))` daje $1{,}0$ (zgodnie z
$(-1)^{5-1}=1$), a `np.sum(...)` daje $\approx-7{,}8\cdot10^{-16}\approx0$ ✓.
