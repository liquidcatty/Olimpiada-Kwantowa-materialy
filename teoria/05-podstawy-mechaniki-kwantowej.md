...
> **Warsztat źródłowy:** „Podstawy mechaniki kwantowej i podstawowe układy kwantowe”
> (Tomasz Sowiński).
> **Czas nauki:** ~6 h teorii + ~5 h zadań.
> **Wymagana wiedza wstępna:** liczby zespolone ([01](01-liczby-zespolone.md)),
> algebra liniowa ([02](02-algebra-liniowa.md)), prawdopodobieństwo
> ([03](03-rachunek-prawdopodobienstwa-i-statystyka.md)) i analiza
> ([04](04-elementy-analizy-matematycznej.md)).

## 1. Po co to jest

To rozdział centralny I etapu: tu pojawiają się **postulaty** i dwa zadania
przykładowe organizatora. Zadanie **P1** (cząstka w nieskończonej studni potencjału)
rozwiązujemy w pełni — to najprostszy układ, w którym widać kwantyzację energii.
Zadanie **P2** (polaryzatory i pojedynczy foton) to kwantowa realizacja **prawa
Malusa** na spinie fotonu. Umiejętność policzenia wartości oczekiwanej, wariancji
i prawdopodobieństw pomiaru jest podstawą wszystkich dalszych rozdziałów.

Po tym rozdziale powinieneś umieć:

1. sformułować **postulaty** i przełożyć je na rachunek (stan, obserwabla, pomiar,
   ewolucja);
2. rozwiązać równanie Schrödingera dla studni, oscylatora i stanów stacjonarnych;
3. policzyć $\langle A\rangle$, $\Delta A$ oraz sprawdzić **zasadę nieoznaczoności**;
4. opisać **spin 1/2** i **polaryzację fotonu** jako kubit oraz wyprowadzić prawo Malusa;
5. zrozumieć **twierdzenie Ehrenfesta** i ewolucję superpozycji (dudnienia).
...
- **Wartość oczekiwana**: $\langle A\rangle=\langle\psi\lvert A\rvert\psi\rangle
  =\int\psi^*A\psi\,dx$; **wariancja** $\operatorname{Var}A=\langle A^2\rangle-\langle A\rangle^2$.
...
> **Ponad program:** korelacje spinów splątanych cząstek prowadzą do nierówności
> Bella (rozdział 09) — ale już tu widzisz, że pomiar „jednej osi” nie wyznacza
...
Iloczyn $\Delta x\,\Delta p=\pi\hbar\sqrt{\tfrac1{12}-\tfrac1{2\pi^2}}\approx0{,}568\,\hbar
\approx1{,}14\cdot\tfrac{\hbar}{2}\ \ge\ \tfrac{\hbar}{2}$.
...
**Z-05.3.** Spin-1/2 w stanie $\lvert\psi\rangle=\cos\tfrac\theta2\lvert\uparrow\rangle+
\sin\tfrac\theta2\lvert\downarrow\rangle$, $\theta=\pi/2$.
...
a rozdział 08 (algorytmy kwantowe) korzysta z całego aparatu wartości oczekiwanych i
ewolucji. Splątanie i twierdzenie Bella rozwinie rozdział 09. Rozwiąż zadania i
porównaj z [rozwiązaniami](../zadania/rozwiazania/rozwiazania-05.md); praca domowa
łącząca rozdziały 01–05 to [PD-1](../praca-domowa/praca-domowa-01.md).

**Kod.** Studnię, oscylator i ewolucję czasową symulujemy numerycznie (diagonalizacja
macierzy hamiltonianu, `np.linalg.eigh`); stany i bramki — w konwencji
[konwencji](../docs/03-konwencje-i-notacja.md) (małoendianowo, $\theta/2$ w obrotach).
`matplotlib` nie jest wymagany do rachunków.